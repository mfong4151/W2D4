...
#concatnae the string each time off the ends
def reverse_string(str)
    return str if str.length <= 1
    
    mid = reverse_string(str[1...-1])
...
def flatten(data)
   ...
   return data if data.all?{|i| i.class != Array}
   res = []
   data.each do |i|
        if i.class == Array
            #res += flatten([*i])
            #flatten([*i]).each{|j| res << j } #optimal memory solution
            res.push(*flatten([*i]))
        else
            res << i 
